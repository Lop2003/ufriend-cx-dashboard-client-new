    ...
    return (
      <div className="flex items-center gap-0.5 text-amber-400">
        {Array.from({ length: 5 }).map((_, i) => (
          <svg
            key={i}
            className={`h-3.5 w-3.5 ${i < rating ? 'fill-current' : 'text-slate-200 fill-slate-200'}`}
            viewBox="0 0 20 20"
          >
        ...
        <div className="flex flex-col gap-3 max-h-[360px] overflow-y-auto pr-0.5">
          {sorted.map((fb) => {
            ...
            return (
              <div
                key={fb.id}
                className="p-4 rounded-2xl bg-slate-50/55 border border-slate-200/60 transition-all hover:bg-slate-50 hover:translate-y-[-1px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.015)]"
              ...
              >
                <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="h-4.5 px-1.5 py-0 text-[8.5px] font-extrabold bg-blue-50 text-[#0051ba] border-blue-100/60 rounded-md"
                    >
                ...
                <p className="text-[12px] text-slate-700 font-bold leading-relaxed mb-3 text-left">
                  "{fb.comment}"
                </p>
