...
export interface DashboardChartBarProps {
...
export function DashboardChartBar({
  ...
  // Reuse same daily data generation logic as Area Chart
  ...
  const branchKeys = useMemo(() => {
    ...
    // Sort by average descending so that the largest branches sit at the bottom of the stack
  ...
  const title = isBranchSelected
    ? `แนวโน้มแท่งรายวัน: สาขา${selectedBranch}`
    : "สัดส่วนสะสมและแนวโน้มลูกค้าแยกตามสาขา"
  const subtitle = isBranchSelected
    ? `แสดงข้อมูลลูกค้าในรูปแบบกราฟแท่งรายวัน (${periodText})`
    : `แสดงสัดส่วนยอดผู้รับบริการสะสมของทุกสาขาในรูปแบบกราฟแท่งซ้อนกัน (${periodText})`
          ...
          >
            ...
            {isBranchSelected ? (
              <Bar
                ...
                fill={`var(--color-${safeSelectedBranch})`}
                fillOpacity={0.6}
              ...
              branchKeys.map((branch, index) => {
                const isTop = index === branchKeys.length - 1
                const colorVar = `var(--color-${branch})`
                return (
                  <Bar
                    key={branch}
                    dataKey={branch}
